...
def predict_whole(model, image, tile_size):
    ...
    x = model(image.to('cuda' if torch.cuda.is_available() else 'cpu'))
    x = interp(x)
    return x
...
def predict_multiscale(model, image, scales=[0.75, 1.0, 1.25, 1.5, 1.75, 2.0], tile_size=(512, 512)):
    ...
    image = image.data.to('cuda' if torch.cuda.is_available() else 'cpu').numpy()

    full_probs = torch.zeros((1, 1, image_size[2], image_size[3])).to('cuda' if torch.cuda.is_available() else 'cpu')  # 初始化全概率矩阵 shape(1024,2048,19)
...
def crf_predict(model, img, size=1024):
    ...
    mask = mask.squeeze(0).permute(1, 2, 0).to('cuda' if torch.cuda.is_available() else 'cpu').numpy()
    mask = mask.astype(np.float32)

    img = img.squeeze(0).permute(1, 2, 0).to('cuda' if torch.cuda.is_available() else 'cpu').numpy()
