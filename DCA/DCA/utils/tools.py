...
def generate_pseudo(model, target_loader, save_dir, n_class=7, pseudo_dict=dict(), logger=None):
    ...
    for image, labels in tqdm(target_loader):
        out = model(image.to('cuda' if torch.cuda.is_available() else 'cpu'))
        logits = out[0] if isinstance(out, tuple) else out
        max_items = logits.max(dim=1)
        label_pred = max_items[1].data.to('cuda' if torch.cuda.is_available() else 'cpu').numpy()
        logits_pred = max_items[0].data.to('cuda' if torch.cuda.is_available() else 'cpu').numpy()
        ...
        np_logits = logits.data.to('cuda' if torch.cuda.is_available() else 'cpu').numpy()
...
if __name__ == '__main__':
    seed_torch(2333)
    s = torch.randn((5, 5)).to('cuda' if torch.cuda.is_available() else 'cpu')
    print(s)
